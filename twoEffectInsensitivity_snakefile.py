...
input_table_prevInsens = "twoEffectPrevInsensitivityParamTable.txt"
params_table = pd.read_csv(input_table_prevInsens, delim_whitespace=True)
...
thr = np.array(['{:.1f}'.format(r) for r in tmpThr], dtype=np.str)
tmpCost = np.round(np.array(params_table["C"]),2)
cost = np.array(['{:.2f}'.format(r) for r in tmpCost], dtype=np.str)
tmpAlphaLarge = np.round(np.array(params_table["al"]),3)
alphaLarge = np.array(['{:.3f}'.format(r) for r in tmpAlphaLarge], dtype=np.str)
#alphaLarge = [trim_trailingzero(x) for x in alphaLarge]
# print(cost)
tmpN = np.array(params_table["Ne"].astype(int))
N = np.array(['{:.0f}'.format(r) for r in tmpN], dtype=np.str)
tmpEnvSD = np.round(np.sqrt(params_table["Ve"]),3)
envSD = np.array(['{:.3f}'.format(r) for r in tmpEnvSD], dtype=np.str)

print(params_table)

rule all:
  input:
    input_table_prevInsens,
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.fixedSmall", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.fixedLarge", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.meanSmall", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.meanLarge", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.mean", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.prev", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.h2l", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.h2s", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.genVar", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.nSegSmall", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.nSegLarge", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.deltaRSmall", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost),
    expand("twoEffectInsensitivity/all/PopSize{N}_aL{alphaLarge}_thr{thr}_envSD{envSD}_cost{cost}_all.deltaRLarge", zip, N=N, alphaLarge = alphaLarge, thr=thr, envSD=envSD, cost=cost)
  params:
     time="36:00:00",
     partition="broadwl",
     mem="4Gb",
     path="twoEffectInsensitivity/all"
  output:
...
rule twoEffectBackup:
  input:
    "twoEffect{suffix}ResultsTable.Rdata"
  params:
    time="36:00:00",
    partition="broadwl",
    mem="2Gb"
  output:
    protected("twoEffect{suffix}ResultsTableBackup.Rdata")
  shell:
    """cp {input} {output}"""    
...
rule result_combined_two_small:
  ...
  output:
    "twoEffect{suffix}/all/PopSize{N}_aL{liaSizes}_thr{thr}_envSD{envsd}_cost{cost}_all.{ext}"
  shell:
     """cat {input} >> {output}"""
