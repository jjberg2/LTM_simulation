...
b.table <- a.table[rep(c(1,2),each=length(Ne)),]
b.table$Ne <- rep(Ne,times=2)
c.table <- b.table[,!colnames(b.table) %in% c('h2s','h2l','prev','h2os','h2ol','deltas','deltal')]



write.table(c.table, "twoEffectPopSizeInsensitivityParamsTable.txt", col.names=T, row.names=F, quote=F)
